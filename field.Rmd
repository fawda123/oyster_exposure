...
dissmod <- dissdat %>% 
  ...
  mutate(
    ...
    estmod = pmap(list(Organism, lmmod), function(Organism, lmmod){
      ...
      out <- list(mnsval = mnsval, cnsval = cnsval)
      
      return(out)
      
    }), 
    plomod = purrr::pmap(list(Organism, estmod), function(Organism, estmod){
      ...
      # mean esimate plots
      p1 <- ggplot(estmod$mnsval, aes(x = Exposure, y = Mean)) + 
        geom_point(size = 3) + 
      ...
      # contrast plots
      p2 <- ggplot(estmod$cnsval, aes(x = Contrast, y = Difference, colour = sig)) +
...
p1 <- ggplot(toplo1, aes(y = comp, x = diff, linetype = pcol, col = Organism, fill = Organism)) + 
  geom_vline(xintercept = 0, color = 'black', linetype = 'dotted') +
  geom_col(position = position_dodge(width = 0.3), alpha = 0.7, color = NA, width = 0.3) + 
...
p2 <- ggplot(toplo2, aes(y = comp, x = diff, linetype = pcol, col = Organism, fill = Organism)) + 
  geom_vline(xintercept = 0, color = 'black', linetype = 'dotted') +
  geom_col(position = position_dodge(width = 0.3), color = NA, width = 0.3, alpha = 0.7) + 
  geom_errorbar(aes(xmin = lwr, xmax = upr), position = position_dodge(width = 0.3), width = 0.2) + 
  facet_wrap(~var, scales = 'free_x', ncol = 2, strip.position = 'bottom', labeller = label_parsed) + 
  scale_linetype_manual(values = c('solid', 'dashed'), drop = F) +
  theme_ipsum(plot_margin = margin(5, 5, 5, 5)) + 
...
p1 <- ggplot(toplo1, aes(y = comp, x = diff, col = Organism, fill = Organism)) + 
  geom_vline(xintercept = 0, color = 'black', linetype = 'dotted') +
  geom_col(position = position_dodge(width = 0.3), alpha = 0.7, color = NA, width = 0.3) + 
  geom_errorbar(aes(xmin = lwr, xmax = upr), position = position_dodge(width = 0.3), width = 0.2) + 
  facet_wrap(~var, scales = 'free_x', ncol = 2, strip.position = 'bottom', labeller = label_parsed) + 
...
jpeg(here('figs/fieldcompa.jpeg'), height = 3, width = 5, units = 'in', res = 500, family = 'serif')
print(p1)
dev.off()
```
...
knitr::include_graphics(here('figs/fieldcompa.jpeg'))
```
...
p1 <- dissmod$plomod[[1]]
p1[[1]] <- p1[[1]] + scale_y_continuous(limits= c(0 ,3))
p1[[2]] <- p1[[2]] + scale_y_continuous(limits= c(-1.25, 2))
p2 <- dissmod$plomod[[2]]
p2[[1]] <- p2[[1]] + scale_y_continuous(limits= c(0 ,3))
p2[[2]] <- p2[[2]] + scale_y_continuous(limits= c(-1.25, 2))
...
toplo <- dissmod %>% 
  select(Organism, estmod) %>% 
  mutate(
    estmod = purrr::map(estmod, function(x){
      ...
      cns <- x[[2]] %>% 
        ...
        select(lev, est, CI_low, CI_high) %>% 
        mutate(plo = 'contrasts')
      
      out <- bind_rows(mns, cns)
  ...
  select(-SE) %>% 
  mutate(
    lev = factor(lev, levels = c('Mid', 'Edge', 'Out', 'Out-Mid', 'Out-Edge', 'Edge-Mid')),
    plo = factor(plo, levels = c('means', 'contrasts'), labels = c('Estimated means (+/-95% CI)', 'Estimated differences (+/- 95% CI)')), 
    pcol = ifelse(0 > CI_low & 0 < CI_high, 'ns', 'p < 0.05'),
    pcol = factor(pcol, levels = c('p < 0.05', 'ns')),
    Organism = factor(Organism, levels = c('Olympia', 'Pacific'))
  ) 

p <- ggplot(toplo, aes(y = lev, x = est, linetype = pcol, col = Organism, fill = Organism)) + 
  geom_vline(xintercept = 0, color = 'black', linetype = 'dotted') +
  geom_col(position = position_dodge(width = 0.3), alpha = 0.7, color = NA, width = 0.3) + 
  geom_errorbar(aes(xmin = CI_low, xmax = CI_high), position = position_dodge(width = 0.3), width = 0.2) + 
  facet_wrap(~plo, scales = 'free', ncol = 2, strip.position = 'bottom') + 
  scale_linetype_manual(values = c('solid', 'dashed'), drop = F) +
  guides(
    fill = guide_legend(override.aes = list(linetype = c(0, 0))), 
    linetype = guide_legend(override.aes = list(fill = c(NA, NA)))
...
jpeg(here('figs/fielddissa.jpeg'), height = 3.25, width = 6, units = 'in', res = 500, family = 'serif')
print(p)
dev.off()
```

```{r, out.width='70%'}
knitr::include_graphics(here('figs/fielddissa.jpeg'))
```
