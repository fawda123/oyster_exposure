...
```{r, results = 'hide'}
toplo <- allexp %>% 
  filter(var %in% c('Final size (cm2)', 'Shell weight (g)', 'Tissue weight (g)')) %>% 
  filter(week == 6) %>% 
...
jpeg('figs/growthvar.jpg', height = 7, width = 7, units = 'in', res = 300, family = 'serif')
print(p)
dev.off()
```
