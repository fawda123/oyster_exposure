---
title: "Modelled pH time series decomposition"
author: "M. W. Beck"
...
```{r}
moddat <- read_excel(here('data/raw/pH for Nina 2021-09-24.xlsx')) %>% 
  select(Date, Time, matches('0:42|0:0|14:21')) %>% 
  gather('cell', 'pH', -Date, -Time) %>% 
  mutate(
    cell = case_when(
      grepl('14:21', cell) ~ '14,21', 
      grepl('0:42', cell) ~ '0,42', 
      grepl('0:0', cell) ~ '0,0'
    ), 
    Date = gsub('^(.*)\\s.*$', '\\1', Date)
  ) %>% 
  ...
  mutate(
    datetime = ymd_hms(datetime, tz = 'Pacific/Pitcairn')
  )
...
phres <- moddat %>% 
  ...
  nest() %>% 
  mutate(
    annprd = pmap(list(cell, data), function(cell, data){
...
pH time series for all cells plus estimated annual component. 
...
Estimated amplitudes for annual, daily, and tidal components for each cell. 
...
Estimated amplitudes for annual, daily, and tidal components for each cell, alternative plot. 
