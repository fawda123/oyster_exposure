...
```{r}
phres <- tibble(
  site = c('elkvm', 'nocec', 'nartb', 'pdbby')
  ) %>% 
  group_by(site) %>% 
  nest() %>% 
  # select(-data) %>% 
  mutate(
    annprd = pmap(list(site, data), function(site, data){
