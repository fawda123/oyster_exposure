...
qntplos <- allexp %>% 
  filter(var %in% c('Delta size (cm2)', 'Final size (cm2)', 'Size rate (cm2/d)', 'Shell weight (g)', 'Tissue weight (g)')) %>% 
  ...
  mutate(
    ...
    qtplo = purrr::pmap(list(data, qtmod, var, week, species), function(data, qtmod, var, week, species){
      ...
      tomod <- data %>% 
        mutate(
          trt = factor(trt, levels = rev(c(trts$shrtlab)), labels = rev(c( 'Intercept (7.7C)', trts$shrtlab[-1])))
        )
      ...
      coeffs <- qtmod$coefficients %>% 
        ...
        mutate(
          trt = gsub('^trt', '', trt), 
          trt = factor(trt, levels = rev(c('(Intercept)', trts$shrtlab[-1])), labels = rev(c( 'Intercept (7.7C)', trts$shrtlab[-1]))), 
  ...
  mutate( 
    qtplo = purrr::pmap(list(data, qtmod, var, species), function(data, qtmod, var, species){
      ...
      coeffs <- qtmod$coefficients %>%
        ...
        mutate(
          ...
          trt = factor(trt,
                       ...
                       labels = c('week6', 'week4', rev(trts$shrtlab[-1]), 'Intercept (week2, trt7.7C)')
                       ), 
      ...
      p <- ggplot(coeffs, aes(y = trt, x = val)) +
        ...
        geom_errorbarh(aes(xmin = val - ci, xmax = val + ci, col = pval), height = 0.2) +
        geom_vline(xintercept = 0, linetype = 'dashed') + 
        scale_colour_manual(values = c('black', 'red')) + 
        theme_minimal() +
        theme(
