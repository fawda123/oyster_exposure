...
Then the velocity data are imported.  ADCP data were collected at two stations at the northern and southern edge of the kelp bed.  These data were pre-processed by Greg (email Jan. 5, 2022) to extract a velocity component along the main axis of the kelp bed as measure of flow in/out of the bed. A low-pass filter to smooth the data using a six hour window was also used. Positive values indicate seaward and negative values indicate landward.  Units are m/s and represent an average across the depth profile.  
...
mapview(kelp,layer.name = 'Kelp area') + 
  mapview(datloc, col.regions = c('red', 'blue'), layer.name = 'Stations')
```
...
The weights used to average NB1 and SB1 vary at each item step from 0-1 depending on the range of values in teh velocity vector.
